...
SEARCH_OPTIONS = ["me", "mce", "kelseysearch"]
...
def main():
    ...
    parser.add_argument('-c', '--changes', type=str, help="list of modifications to make to model.")
    ...
    pr_obj = Problem(args.changes, args.model, args.nmodel, args.problem, args.tmodel,
     args.ground, args.approx, args.heuristic,
     args.tproblem, args.hproblem, args.plan_file)

    #Ky Added 
    if args.search.lower() == "kelseysearch":
        plan = pr_obj.KySearch()
        print("Ky Search plan", plan)
    elif args.search.lower() == "me":
