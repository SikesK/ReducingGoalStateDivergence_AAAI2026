...
def apply_modifications(mod_list, domainFileName, problemFileName):
    ...
    for key in action_dict.keys():
        ...
        for effect in action_dict[key][3].items():
            print("im an effect oh yeahhh", effect)
            if effect[1]: state.append('{}-has-add-effect-{}'.format(actionName, effect[0]))
            else:         state.append('{}-has-delete-effect-{}'.format(actionName, effect[0]))
    for pred in init_state:
        print("im an pred in init state", pred)
        state.append('has-initial-state-{}'.format(pred))
    for pred in goal_state:
        state.append('has-goal-state-{}'.format(pred))

    print("state before modificatoins", state)

    #new_initial_state = []
    new_child_states_list = []
    #you're looping through second list item twice?

    for modification in mod_list[1:]: # [ [] , [A] ]
        print("modification", modification)
        if not modification:
            continue
        else:
            new_state = state.copy()
            for element in modification: #each change to make 
                if element in new_state:
                    print(element," is getting removed")
                    new_state.remove(element)
                else:
                    print(element," is not in the model")
            new_child_states_list.append(new_state)
    #print("new child states list", new_child_states_list)

    print("heyyyy im your state", new_state)
...
def parse_problem(domainFileName, problemFileName, section_prefix):
    ...
    #print("parse problem state", state)
    return state
