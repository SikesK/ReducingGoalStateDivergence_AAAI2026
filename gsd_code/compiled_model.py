...
def find_max_gsd(path_to_human_domain, path_to_human_problem, path_to_robot_domain, path_to_robot_problem, designs, max_div_budget=50):
    #calculate the max gsd for the given problem pair
    ...
    time_step_preds = []
    for i in range(max_div_budget):
        other_predicates.append(Predicate("time_step_" + str(i)))
        time_step_preds.append(Predicate("time_step_" + str(i)))
   ...
   # Add a flip robot can action action that checks whether robot goal is met and then deletes robot_can_act
    ...
    for time_step in range(max_div_budget):
        for pred in list_h_preds:
            #check_actions.append(ACTION_TEMPLATE_WITH_COST.format(action_name="check_act" + str(pred.name),preconditions="(r_" + str(pred.name)+") ("+pred.name+") (not (robot_can_act)) (not (human_can_act))", effects="(check_" + str(pred.name)+")", cost="10"))
            #check_actions.append(ACTION_TEMPLATE_WITH_COST.format(action_name="check_act_not_" + str(pred.name),preconditions="(not (r_" + str(pred.name)+")) (not ("+pred.name+")) (not (robot_can_act)) (not (human_can_act))", effects="(check_" + str(pred.name)+")", cost="10"))
            check_actions.append(ACTION_TEMPLATE.format(action_name="check_disagree_act1" + str(pred.name) + str(time_step),preconditions="(not (r_" + str(pred.name)+")) ("+pred.name+") (not (robot_can_act)) (not (human_can_act))(not (check_" + str(pred.name)+"))", effects="(and (time_step_" + str(time_step)+") (check_" + str(pred.name)+"))"))
            check_actions.append(ACTION_TEMPLATE.format(action_name="check_disagree_act2" + str(pred.name) + str(time_step),preconditions=" (r_" + str(pred.name)+") (not ("+pred.name+")) (not (robot_can_act)) (not (human_can_act)) (not (check_" + str(pred.name)+"))", effects="(and (time_step_" + str(time_step)+") (check_" + str(pred.name)+"))"))


    all_actions = new_human_action + list_r_actions +  [flip_human_action, flip_robot_action] #+ check_actions
...
if __name__ == '__main__':
    ...
    for budget in range(10, number_of_predicate, 20):
